...
import { Tooltip, keymap, EditorView, showTooltip } from "@codemirror/view";
...
export const createQuickEditToolTip = (
  state: EditorState,
): readonly Tooltip[] => {
  ...
  return [
    {
      ...
      create() {
        ...
        dom.className =
          "bg-popover text-popover-background z-50 rounded-sm border border-input p-2 shadow-md flex flex-col gap-2 text-sm";

        const form = document.createElement("form");
        form.className = "flex flex-col gap-2";
        const input = document.createElement("input");
        input.type = "text";
        input.placeholder = "Edit the selected code";
        input.className =
        ...
        cancelButton.className =
          "font-sans py-1 px-2 text-muted-foreground hover:text-foreground hover:bg-foreground/10 rounded-sm";
